...
namespace XpressNetSharp.Requests.LocomotiveFunctions.AddressInquiry.MultiUnitMember
{
    ...
    /// </summary>
    /// <remarks>
    /// If the locomotive addresses within an MTR is not known, set the address to 0 and the first address within the MTR will be
    /// returned.
    /// </remarks>
    public class ForwardSearchMessage : Message
    {
        /// <summary>
        /// Requests the next member in a multi-unit consist.
        /// </summary>
        /// <remarks>
        /// If the locomotive addresses within an MTR is not known, set the address to 0 and the first address within the MTR will be
        /// returned.
        ...
        public ForwardSearchMessage(int mtr, int address) 
            : base(PacketHeaderType.LocomotiveFunction) 
        {
            Payload.Add(Convert.ToByte(PacketIdentifier.LocomotiveFunctionRequest.AddressInquiryMultiUnitMember_ForwardSearch));
